...
import '../../../../core/network_connection_checker/new_connection_check.dart';
...
class _AttendanceRegularisationDatePickState
  ...
  void _onRangeSelected(DateTime? start, DateTime? end, DateTime focusedDay) {
    ...
    if (start != null) {
      // Increment counter for each new request
      _requestCounter++;
      _fetchAttendanceData(start, end ?? start, _requestCounter);
    }
  }
  ...
  Future<bool> _checkNetworkConnectivity() async {
    try {
      // You can use connectivity_plus package or your custom network check
      // For now, assuming you have a utility function
      final hasNetwork = await checkInternetConnection();
      return hasNetwork;
    } catch (e) {
      return false;
    }
  ...
  Future<void> _showNoInternetPopup() async {
    showNoInternetPopup(
      ...
      onPressed: () async {
        ...
        // Retry the last request after closing popup
        if (_lastRequestStart != null) {
          _requestCounter++;
          await _fetchAttendanceData(
            _lastRequestStart!,
            _lastRequestEnd ?? _lastRequestStart!,
            _requestCounter,
          );
        }
      },
  ...
  Future<void> _fetchAttendanceData(
    ...
    // Don't process if loading and it's the same date range
    if (_isLoading &&
        _lastRequestStart == startDate &&
        _lastRequestEnd == endDate) {
      return;
    }
    ...
    try {
      ...
      if (requestId == _requestCounter && response != null) {
  ...
  Future<void> refreshCalendarData() async {
    ...
    if (_rangeStart != null) {
      ...
      await _fetchAttendanceData(_selectedDay!, _selectedDay!, currentRequest);
    }
  }
...
class _AttendanceRegularisationDatePickState
  ...
  // Replace your _onRangeSelected method with this:
  void _onRangeSelected(DateTime? start, DateTime? end, DateTime focusedDay) {
    setState(() {
      _selectedDay = null;
      _focusedDay = focusedDay;
  ...
  // Update _fetchAttendanceData to handle race conditions:
  Future<void> _fetchAttendanceData(
    DateTime startDate,
    DateTime endDate,
    int requestId, // Add request ID parameter
  ) async {
    // Don't process if loading and it's the same date range
    if (_isLoading &&
        _lastRequestStart == startDate &&
  ...
  // Also update refreshCalendarData:
  Future<void> refreshCalendarData() async {
    _requestCounter++;
    final currentRequest = _requestCounter;
